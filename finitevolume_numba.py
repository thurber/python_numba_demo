...
def roll(M, direction, axis=0):
    temp = np.empty(M.shape, M.dtype)
    if axis == 0:
        for i in prange(M.shape[1]):
            temp[:, i] = np.roll(M[:, i], direction)
    else:
...
def getGradient(f, dx):
	...
	f_dx = ( roll(f,R,axis=0) - roll(f,L,axis=0) ) / (2*dx)
	f_dy = ( roll(f,R,axis=1) - roll(f,L,axis=1) ) / (2*dx)
...
def slopeLimit(f, dx, f_dx, f_dy):
	...
	f_dx = np.maximum(0., np.minimum(1., ( (f-roll(f,L,axis=0))/dx)/(f_dx + 1.0e-8*(f_dx==0)))) * f_dx
	f_dx = np.maximum(0., np.minimum(1., (-(f-roll(f,R,axis=0))/dx)/(f_dx + 1.0e-8*(f_dx==0)))) * f_dx
	f_dy = np.maximum(0., np.minimum(1., ( (f-roll(f,L,axis=1))/dx)/(f_dy + 1.0e-8*(f_dy==0)))) * f_dy
	f_dy = np.maximum(0., np.minimum(1., (-(f-roll(f,R,axis=1))/dx)/(f_dy + 1.0e-8*(f_dy==0)))) * f_dy
